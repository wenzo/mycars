<template>
  <ion-page>
    ...
    <ion-content style="--padding-bottom: calc(var(--ion-tab-bar-height, 56px) + var(--ion-safe-area-bottom, 0px))">
      ...
      <!-- ═══ PROFILO ══════════════════════════════════════════════════════ -->
      <template v-else-if="activeTab === 'profilo'">
        <div class="profilo-wrap">
          ...
          <div class="profilo-card" style="padding:14px 16px">
            ...
            <div v-else>
              ...
              <button v-if="clientStore.requests.length > 3" class="link-btn" @click="activeTab = 'noleggi'">
...
.profilo-avatar-card {
  background: linear-gradient(135deg, var(--dealer-secondary), color-mix(in srgb, var(--dealer-secondary) 70%, #000));
...
.field-input:focus { border-color: var(--dealer-secondary); }
.save-btn {
  height: 46px; background: var(--dealer-secondary); color: #fff; border: none;
...
.di-row ion-icon { font-size: 16px; color: var(--dealer-secondary); flex-shrink: 0; }
.di-row a { color: var(--dealer-secondary); text-decoration: none; }
