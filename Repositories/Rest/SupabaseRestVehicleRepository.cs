...
public sealed class SupabaseRestVehicleRepository : IVehicleRepository
{
        ...
        Guid operatorId, IReadOnlyList<Guid> ids, CancellationToken ct = default)
    {
        ...
        var items   = await _db.SelectAsync<VehicleCard>(
            "public_vehicle_cards",
            $"operator_id=eq.{operatorId}&id=in.({idList})",
            limit: ids.Count);
        // Riordina per corrispondenza con l'ordinamento di rilevanza della ricerca vettoriale
        var order = ids.Select((id, i) => (id, i)).ToDictionary(x => x.id, x => x.i);
        return items.OrderBy(v => order.TryGetValue(v.Id, out var i) ? i : 999).ToList();
    }
}
